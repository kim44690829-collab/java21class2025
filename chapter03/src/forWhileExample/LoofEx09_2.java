		...
		while(run == true) {
			...
			if(loginMenu == 1) { // 로그인 버튼 클릭
				...
				if(login == false) {
					...
					charge1 = dbMoney1 - withdraw;
					charge2 = dbMoney2 - withdraw;
					if(acc == dbAcc1) {
						if(charge1 < 0) {
				...
				if(login == false) {
					...
					if(acc == dbAcc1) {
						...
						if(depositAcc == dbAcc1) {
							System.out.println("본인의 계좌는 선택할 수 없습니다.");
						}else if(depositAcc == dbAcc2){
						...
						System.out.print("입금할 계좌를 입력해주세요 : ");
						depositAcc = scan.nextInt();
						if(depositAcc == dbAcc2) {
							System.out.println("본인의 계좌는 선택할 수 없습니다.");
						}else if(depositAcc == dbAcc1){
							System.out.print("이체할 금액을 입력해주세요 : ");
							depositMoney = scan.nextInt();
			...
			}else if(loginMenu == 0) { // 종료 버튼 클릭
				System.out.println("프로그램을 종료합니다.");
				run = false;
			} else { // 1~3 이외의 버튼 클릭
				System.out.println("0번부터 6번까지의 번호중 하나를 선택해주세요");
			}
		}
	}
}
